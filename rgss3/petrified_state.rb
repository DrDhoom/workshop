...
class Window_BattleEnemy < Window_Selectable  
  ...
  def current_item_enabled?
    if BattleManager.actor && BattleManager.actor.input.item
      return false if BattleManager.actor.input.item.is_a?(RPG::Item) && enemy.petrified?(:all) && !BattleManager.actor.input.item.ignore_petrified
...
class Window_BattleActor < Window_BattleStatus
  ...
  def current_item_enabled?    
    if BattleManager.actor && BattleManager.actor.input.item
      return false if BattleManager.actor.input.item.is_a?(RPG::Item) && $game_party.battle_members[@index].petrified?(:all) && !BattleManager.actor.input.item.ignore_petrified
      return false if $game_party.battle_members[@index].petrified?(BattleManager.actor.input.item.stype_id) && !BattleManager.actor.input.item.ignore_petrified
    end
